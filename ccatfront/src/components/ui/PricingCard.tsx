...
export default function PricingCard({
  ...
  return (
    <div className={`relative bg-white rounded-2xl border p-8 flex flex-col gap-5 ${recommended ? 'border-blue-400 shadow-lg' : 'border-gray-200'}`}>
        ...
        {features.map((f, i) => (
          <li key={i} className="flex items-center gap-2 text-sm text-gray-700">
            {f.included
              ...
              : <XCircle size={17} className="text-gray-300 shrink-0" />}
            {f.text}
          </li>
        ))}
      </ul>
